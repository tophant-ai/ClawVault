...
class ClawVaultManager:
    ...
    def _pip_install(self, *args: str) -> subprocess.CompletedProcess:
        """Run pip install inside the venv with a stderr heartbeat every 15s.

        pip itself can stay silent for 30+ seconds while resolving or downloading
        heavy deps (mitmproxy, cryptography, ...). This emits periodic progress
        lines to stderr so stream watchdogs (e.g. OpenClaw's 30s idle monitor)
        don't flag the install as dead. stdout stays clean for --json output.
        """
        cmd = [
        ...
        def heartbeat() -> None:
            ...
            while not done.wait(15):
                print(
                    f"  …pip install still running ({int(time.time() - start)}s)",
    ...
    def install(
        ...
        *,
        ...
        # 3. Install from GitHub (main branch — always the latest code)
        print("📦 Installing from GitHub (main branch)...")
    ...
    def run_tests(self, category: str = "all") -> dict:
        ...
        try:
            ...
            for test in test_cases:
                ...
                finding_count = result.total_detections
                results.append({
                    "name": test["name"],
